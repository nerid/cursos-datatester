  ...
  return (
    <div className="min-h-screen bg-[var(--color-hornette-bg)]">
      ...
      <main className="max-w-4xl mx-auto px-6 py-12">
        <div className="bg-[var(--color-hornette-card)] rounded-2xl p-8 border border-white/10 premium-shadow mb-8">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-4">
                    {isCompleted ? (
                      <CheckCircle className="w-6 h-6 text-[var(--color-hornette-primary)]" />
                    ) : (
                      <Circle className="w-6 h-6 text-[var(--color-hornette-muted)]" />
                    )}
                    <div>
                      <h3 className={`font-semibold ${isCompleted ? "text-white" : "text-[var(--color-hornette-muted)]"}`}>
                        Sesión {session.id}: {session.title}
                      </h3>
                      <p className="text-xs text-[var(--color-hornette-muted)] mt-1">{session.duration}</p>
                    </div>
                  </div>
                </motion.div>
              );
        ...
        {completedSessions.length > 0 && (
          <motion.div 
            ...
            className="bg-gradient-to-r from-[var(--color-hornette-card)] to-[#1f1f1f] rounded-2xl p-8 border border-white/10 premium-shadow"
          >
